...
Arguments parse_args(int argc, char *argv[]) {
    Arguments retval{};
    if (argc >= 2)
        retval.input_path = argv[1];
    if (argc >= 3)
        retval.output_path = argv[2];
    return retval;
}

// RAII for source and destination files
struct FileHandler {
    FileHandler(const std::string &path, const std::string &&mode) { file = fopen(path.c_str(), mode.c_str()); }
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Copied " << metrics.total_size << " bytes in " << metrics.total_ms << " ms with average speed "
              << rate << " MB/s \n";
    return 0;
}
