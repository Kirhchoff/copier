...
// Provided mainly to avoid too much memory allocations compared to using some standard containers with dynamic size
// (like vector/queue) No constraints for size or data type stored are implemented. Will allocate all the required
// memory up-fromt. Use with care. Not suitable for single-thread application. Requires separate reader and writer
// threads.

// TODO: Could it be made simpler by using std::deque with custom allocator / pmr?
template <class DataPiece, unsigned int SIZE> class CircularBuffer {
  public:
    ...
    bool is_empty() const { return free_space == SIZE; }
